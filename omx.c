...
static av_cold int omx_component_init(AVCodecContext *avctx, const char *role)
{
    ...
    vid_param_inline_header.nSize = sizeof(OMX_CONFIG_PORTBOOLEANTYPE);
	vid_param_inline_header.nVersion.nVersion = OMX_VERSION;
	...
	OMX_PARAM_BRCMVIDEOAVCSEIENABLETYPE  vid_param_sei_enable = {0, };
	INIT_STRUCT(vid_param_sei_enable);
    vid_param_sei_enable.nPortIndex = s->out_port;
    vid_param_sei_enable.nSize = sizeof(OMX_PARAM_BRCMVIDEOAVCSEIENABLETYPE);
	vid_param_sei_enable.nVersion.nVersion = OMX_VERSION;
    vid_param_sei_enable.bEnable = OMX_TRUE;
    err = OMX_SetParameter(s->handle, OMX_IndexParamBrcmVideoAVCSEIEnable, &vid_param_sei_enable);
	if (err != OMX_ErrorNone)
        av_log(avctx, AV_LOG_WARNING, "Unable to set video sei message\n");	
	
    OMX_CONFIG_PORTBOOLEANTYPE vid_param_sei_timing = {0, };
	INIT_STRUCT(vid_param_sei_timing);
    vid_param_sei_timing.nPortIndex = s->out_port;
    vid_param_sei_timing.nSize = sizeof(OMX_CONFIG_PORTBOOLEANTYPE);
	vid_param_sei_timing.nVersion.nVersion = OMX_VERSION;
    vid_param_sei_timing.bEnabled = OMX_TRUE;
    err = OMX_SetParameter(s->handle, OMX_IndexParamBrcmVideoAVCSPSTimingEnable, &vid_param_sei_timing);
	if (err != OMX_ErrorNone)
        av_log(avctx, AV_LOG_WARNING, "Unable to set video sei timing enable\n");
	*/
